...
class ToolStream:
    ...
    Granularity is one PARAMETER, not one character: the wire carries a value
    untyped and the JSON fragment cannot be built until the value is closed. OpenAI's own format allows arguments fragments of any size, so
    this is conformant — it just cannot be finer than the format permits.
    """
    ...
    def __init__(self, tools=None, make_id=new_call_id):
        ...
        self.calls = []     # per-index: {name, n_params, closed}
        ...
        for i, (body, complete) in enumerate(self._blocks(full)):
            ...
            for k, raw in params[st["n"]:]:
                frag = ("{" if st["n"] == 0 else ", ") + \
                    ...
                    json.dumps(coerce(raw, fn_types.get(k)), ensure_ascii=False)
